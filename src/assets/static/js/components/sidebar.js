...
const calculateChildrenHeight = (el, deep = false) => {
  ...
  for (let i = 0; i < el.childElementCount; i++) {
    ...
    const link = child.querySelector('.submenu-link')
  ...
  height += link.clientHeight
    ...
    if (deep && child.classList.contains('has-sub')) {
      ...
      if (subsubmenu.classList.contains('submenu-open')) {
        const childrenHeight = [...subsubmenu.querySelectorAll('.submenu-link')]
  .reduce((acc, curr) => acc + (curr ? curr.clientHeight : 0), 0)
